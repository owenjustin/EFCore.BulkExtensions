...
namespace EFCore.BulkExtensions
{
    public class TableInfo
    {
        ...
        public void LoadData<T>(DbContext context, Type type, IList<T> entities, bool loadOnlyPKColumn)
        {
            ...
            var ownedTypes = navigations.Where(a => a.DeclaringEntityType.IsOwned());
            ...
            else
            {
                ...
                foreach (var navigation in entityType.GetNavigations().Where(a => !a.IsCollection() && !a.DeclaringEntityType.IsOwned()))
                {
        ...
        protected async Task<int> GetNumberUpdatedAsync(DbContext context, CancellationToken cancellationToken, bool isAsync)
        {
            ...
            if (isAsync)
            {
                await context.Database.ExecuteSqlCommandAsync(sqlSetResult, new object[] { resultParameter }, cancellationToken).ConfigureAwait(false);
            }
            else
            {
                context.Database.ExecuteSqlCommand(sqlSetResult, resultParameter);
            }
