    ...
    public class SqlOperationsServerAdapter : ISqlOperationsAdapter
    {
        ...
        protected async Task MergeAsync<T>(DbContext context, Type type, IList<T> entities, TableInfo tableInfo, OperationType operationType, Action<decimal> progress, CancellationToken cancellationToken, bool isAsync) where T : class
        {
            ...
            if (tableInfo.TimeStampColumnName != null)
            {
                ...
                if (isAsync)
                {
                    await context.Database.ExecuteSqlCommandAsync(sqlAddColumn, cancellationToken).ConfigureAwait(false);
                }
                else
                {
            ...
            if (tableInfo.CreatedOutputTable)
            {
                ...
                if (isAsync)
                {
                    await context.Database.ExecuteSqlCommandAsync(sqlCreateOutputTableCopy, cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    context.Database.ExecuteSqlCommand(sqlCreateOutputTableCopy);
                }
                ...
                if (operationType == OperationType.InsertOrUpdateOrDelete)
                {
                    ...
                    if (isAsync)
                    {
                        await context.Database.ExecuteSqlCommandAsync(sqlAlterTableColumnsToNullable, cancellationToken).ConfigureAwait(false);
                    }
                    else
                    {
                        context.Database.ExecuteSqlCommand(sqlAlterTableColumnsToNullable);
                    }
                }
            }
            ...
            try
            {
                ...
                if (keepIdentity && tableInfo.HasIdentity)
                {
                    ...
                    {
                        await context.Database.OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
                        await context.Database.ExecuteSqlCommandAsync(sqlSetIdentityInsertTrue, cancellationToken).ConfigureAwait(false);
                    }
                    else
                    {
                        context.Database.OpenConnection();
                        context.Database.ExecuteSqlCommand(sqlSetIdentityInsertTrue);
                    }
                }
                ...
                if (isAsync)
                {
                    await context.Database.ExecuteSqlCommandAsync(sqlMergeTable.sql, sqlMergeTable.parameters, cancellationToken).ConfigureAwait(false);
                }
                else
                {
                    context.Database.ExecuteSqlCommand(sqlMergeTable.sql, sqlMergeTable.parameters);
                }
            ...
            finally
            {
                if (!tableInfo.BulkConfig.UseTempDB)
                {
                    if (tableInfo.CreatedOutputTable)
                    {
                        ...
                        if (isAsync)
                        {
                            await context.Database.ExecuteSqlCommandAsync(sqlDropOutputTable, cancellationToken).ConfigureAwait(false);
                        }
                        else
                        {
                            context.Database.ExecuteSqlCommand(sqlDropOutputTable);
                        }
                ...
                if (keepIdentity && tableInfo.HasIdentity)
                {
                    ...
                    if (isAsync)
                    {
                        await context.Database.ExecuteSqlCommandAsync(sqlSetIdentityInsertFalse, cancellationToken).ConfigureAwait(false);
                    }
                    else
                    {
                        context.Database.ExecuteSqlCommand(sqlSetIdentityInsertFalse);
                    }
                    context.Database.CloseConnection();
                }
            }
        ...
        private static DataTable InnerGetDataTable<T>(DbContext context, ref Type type, IList<T> entities, TableInfo tableInfo)
        {
            ...
            var entityNavigationOwnedDict = entityType.GetNavigations().Where(a => a.DeclaringEntityType.IsOwned()).ToDictionary(a => a.Name, a => a);
            ...
            foreach (var entity in entities)
            {
                ...
                foreach (var property in propertiesToLoad)
                {
                    ...
                    else if (entityShadowFkPropertiesDict.ContainsKey(property.Name))
                    {
                        ...
                        columnsDict[columnName] = propertyValue == null ? null : foreignKeyShadowProperty.FindPrincipal().PropertyInfo.GetValue(propertyValue); // TODO Check if can be optimized
                    }
